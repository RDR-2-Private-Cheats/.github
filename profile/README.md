...
```mermaid
flowchart TD
    A[Start Private Suite] --> B{Scan Game State}
    B -->|Stable| C[Render Overlays]
    B -->|Offsets Shifted| D[Auto-Recalibration]
    D --> C
    C --> E{User Adjustments?}
    E -->|Preset Switch| F[Apply New Profile]
    E -->|None| C
    F --> C
```
